...
def multiprocess(num_procs, G, node_id_list):
    def get_closeness(G, node_id):
        return snap.GetClosenessCentr(G, node_id)

    pool = mp.Pool(processes=num_procs)
    apply_results = {id: pool.apply_async(get_closeness, args=(G, id)) for id in node_id_list}
    results = {id: apply_results[id].get() for id in apply_results}
    return results
...
num_nodes = G_snap.GetNodes()

# setup threading
start_time = timeit.default_timer()
closeness_dict = multiprocess(NUM_PROCS, G_snap, node_id_list)
elapsed = timeit.default_timer() - start_time

# get top closeness centrality
...
out_filename = fn_parts[0] + '.' + fn_parts[1] + '.' + fn_parts[2] + '_closeness_snap_sub'
